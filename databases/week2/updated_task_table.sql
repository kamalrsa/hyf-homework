...
select * from task;
-- How many taska are in the table
select count(id) from task;

-- how many task in the task table dont have valid due date
...
-- select id, title, description, created, updated, due_date, status_id, user_id from task order by created desc limit 1;
select * from task order by created desc limit 1;

-- get the title and due date of all tasks where title or description contains database.
select title, due_date from task where title  like '%database%' or description like '%database%';

-- get the title and status (as text) of all tasks. 
select  task.title, status.name from status inner join task on status.id=task.status_id;

-- get the name of each status, along with a count of how many tasks have the status
...
-- second week
-- Add a task
insert into task ( title, description, created, updated, due_date, status_id, user_id) values ('Car Cleen', 'how to wash the car.', '2017-10-25 06:54:16', '2017-10-15 13:05:09', null, 2, 1);

-- change the title of the task
update task set title = 'description to car wash' where user_id = 1;


-- change the task due date
update task set due_date = '2018-10-25 06:54:16' where id = 1;
...
select title, email  from task join user on user.id = task.user_id where user.email like'%spotify.com';

-- get all the task for 'Donald Duck' with status 'Not started'
select task.title, status.name from task join status on task.status_id = status.id
left join user on task.user_id = user.id
where user.name like'Donald Duck' and task.status_id like 1;

-- get all the task for 'Maryrose Meadows' that where created in september
select title,  task.created from task join user on user.id = task.user_id where user.name like 'Maryrose Meadows' and month(task.created) = 9;

-- find how many tasks where created in each month e.g how many tasks were created in October
-- select count(title) from task where month(task.created) = 9;

select monthname(task.created) as createdMonth, count(task.created) as Totaltaks from task group by monthname(task.created);
